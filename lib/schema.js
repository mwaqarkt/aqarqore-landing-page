import { SITE_URL, abs, CONTACT } from './site';
...
const organization = (locale) => ({
  ...
  email: CONTACT.email,
  // WhatsApp is the only messaging channel; there is no published voice line,
  // so contactType reflects that rather than claiming a telephone service.
  contactPoint: [
    {
      '@type': 'ContactPoint',
      contactType: 'sales',
  ...
  // NOTE: `address` is deliberately omitted until a registered postal address
  // exists. It is optional; the graph stays valid without it.
});
