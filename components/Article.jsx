...
export function AnswerBlock({ children }) {
  return (
    <p className="rounded-2xl border-l-4 border-[#1078C0] bg-blue-50/70 px-5 py-4 text-[15px] leading-relaxed text-slate-800">
      {children}
    </p>
  );
}
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 shadow-sm">
        <table className="w-full text-left text-sm border-collapse min-w-[640px]">
          <thead className="bg-slate-50">
            <tr>
              {head.map((h) => (
                <th key={h} scope="col" className="px-4 py-3 font-bold text-slate-900 border-b border-slate-200 whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
...
export default function Article({ title, lede, updated, readTime, toc = [], children }) {
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 py-14 sm:py-20">
      <header className="space-y-5">
        ...
        <p className="text-xs font-mono text-slate-500">
          AqarQore · Updated {updated}
          {readTime ? ` · ${readTime}` : ''}
        </p>
      </header>

      {toc.length > 0 && (
        <nav aria-label="On this page" className="mt-9 rounded-2xl border border-slate-200 bg-slate-50/70 p-5">
          <p className="text-[11px] font-mono font-bold uppercase tracking-widest text-slate-500">On this page</p>
